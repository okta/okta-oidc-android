...
import static com.okta.oidc.storage.OktaRepository.EncryptionException.BIOMETRIC_AUTHENTICATION;
...
import static com.okta.oidc.storage.OktaRepository.EncryptionException.KEYGUARD_AUTHENTICATION;
...
public class OktaRepository {
            ...
            if (!requireHardwareBackedKeyStore || encryptionManager != null &&
                ...
                if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    try {
                        ...
                        throw new EncryptionException(ENCRYPT_ERROR, error, e);
                    } catch (GeneralSecurityException e) {
                        throw new EncryptionException(INVALID_KEYS_ERROR,
                                e.getMessage(), e);
                    }
                } else {
                    try {
                ...
                throw new EncryptionException(HARDWARE_BACKED_ERROR, "Client require hardware " +
                        "backed keystore, but EncryptionManager doesn't support it.", null);
            }
            if (cacheMode) {
            ...
            if (cacheMode && cacheStorage.get(key) != null) {
                ...
                if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    try {
                        ...
                        throw new EncryptionException(KEYGUARD_AUTHENTICATION, error, e);
                    } catch (IllegalBlockSizeException keyException) {
                        String error = keyException.getCause().getMessage();
                        if (error != null && error.equals("Key user not authenticated")) {
                            throw new EncryptionException(BIOMETRIC_AUTHENTICATION, error,
                                    keyException);
                        }
                        throw new EncryptionException(INVALID_KEYS_ERROR, error, keyException);
                    } catch (GeneralSecurityException e) {
                        throw new EncryptionException(INVALID_KEYS_ERROR, e.getMessage(), e);
                    }
                } else {
                    try {
                        data = getDecrypted(data);
    ...
    public static class EncryptionException extends Exception {
        ...
        public final static int BIOMETRIC_AUTHENTICATION = 2;
        public final static int KEYGUARD_AUTHENTICATION = 3;
        public final static int HARDWARE_BACKED_ERROR = 4;
        public final static int INVALID_KEYS_ERROR = 5;
